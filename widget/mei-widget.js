...
(function () {
  ...
  var STYLE = "\n" +
    ...
    "#mei-root { position: fixed; z-index: 999999; bottom: 20px; right: 20px; font-family: Georgia, 'Iowan Old Style', 'Palatino Linotype', serif; }\n" +
    ...
    "#mei-bubble { position: absolute; bottom: 76px; right: 0; max-width: 220px; background: var(--mei-cream); color: var(--mei-text); border: 1px solid var(--mei-gold); border-radius: var(--mei-radius); padding: 10px 14px; font-size: 14px; box-shadow: 0 6px 18px rgba(51,57,47,0.18); display: flex; align-items: center; gap: 8px; }\n" +
    "#mei-bubble button { background: none; border: none; color: var(--mei-sage-dark); font-size: 16px; cursor: pointer; line-height: 1; padding: 0; }\n" +
    "#mei-panel { position: absolute; bottom: 76px; right: 0; width: 360px; max-width: calc(100vw - 40px); height: 520px; max-height: 70vh; background: var(--mei-cream); border-radius: var(--mei-radius); box-shadow: 0 12px 32px rgba(51,57,47,0.3); display: none; flex-direction: column; overflow: hidden; border: 1px solid var(--mei-sage-light); }\n" +
    "#mei-panel.mei-open { display: flex; }\n" +
    "#mei-header { background: var(--mei-sage); color: #fff; display: flex; align-items: center; gap: 10px; padding: 12px 14px; }\n" +
    "#mei-header img { width: 36px; height: 36px; border-radius: 50%; border: 2px solid var(--mei-gold); object-fit: cover; }\n" +
    "#mei-header-text { flex: 1; }\n" +
    "#mei-header-text .mei-name { font-weight: bold; font-size: 15px; }\n" +
    "#mei-header-text .mei-sub { font-size: 11px; opacity: 0.85; }\n" +
    "#mei-close { background: none; border: none; color: #fff; font-size: 20px; cursor: pointer; padding: 4px 8px; line-height: 1; }\n" +
    "#mei-body { flex: 1; overflow-y: auto; padding: 14px; display: flex; flex-direction: column; gap: 10px; background: var(--mei-cream); }\n" +
    ".mei-msg { max-width: 85%; padding: 9px 12px; border-radius: 14px; font-size: 14px; line-height: 1.4; white-space: pre-wrap; }\n" +
    ...
    ".mei-btn { background: #fff; border: 1.5px solid var(--mei-sage); color: var(--mei-sage-dark); border-radius: 999px; padding: 9px 14px; font-size: 13.5px; cursor: pointer; text-align: center; text-decoration: none; display: block; }\n" +
    ...
    ".mei-form input { border: 1.5px solid var(--mei-sage-light); border-radius: 10px; padding: 9px 12px; font-size: 14px; font-family: inherit; }\n" +
    ".mei-form input:focus { outline: none; border-color: var(--mei-sage); }\n" +
    "#mei-footer { border-top: 1px solid var(--mei-sage-light); padding: 10px; display: flex; gap: 8px; background: #fff; }\n" +
    "#mei-input { flex: 1; border: 1.5px solid var(--mei-sage-light); border-radius: 999px; padding: 9px 14px; font-size: 14px; font-family: inherit; }\n" +
    "#mei-input:focus { outline: none; border-color: var(--mei-sage); }\n" +
    "#mei-send { background: var(--mei-sage); color: #fff; border: none; border-radius: 999px; padding: 0 16px; font-size: 13.5px; cursor: pointer; }\n" +
    "#mei-send:disabled { opacity: 0.5; cursor: default; }\n" +
    ".mei-typing { font-size: 12.5px; color: var(--mei-sage-dark); font-style: italic; align-self: flex-start; }\n" +
    "@media (max-width: 640px) {\n" +
    "  #mei-root { bottom: 16px; right: 16px; }\n" +
    "  #mei-panel.mei-open { position: fixed; inset: 0; width: 100%; height: 100%; max-width: 100%; max-height: 100%; border-radius: 0; }\n" +
    "  #mei-bubble { right: 0; }\n" +
    "}\n";
  ...
  function build() {
    ...
    var closeBtn = el("button", { id: "mei-close", "aria-label": "Close chat" });
    closeBtn.textContent = "x";
    var header = el("div", { id: "mei-header" }, [headerImg, headerText, closeBtn]);
  ...
  function wireEvents() {
    ...
    els.bubbleDismiss.addEventListener("click", hideBubble);
